...
const releases = await fetch("https://api.github.com/repos/Project-Colony/SphereCord/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "gg.spherecord.app.metainfo.xml");
...
await fs.writeFile("./dist/gg.spherecord.app.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/gg.spherecord.app.metainfo.xml");